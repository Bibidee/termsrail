...
export async function writeAndRead<T>(address: `0x${string}`, provider: Eip1193, functionName: string, args: unknown[], readback: () => Promise<T>, expected: (value: T) => boolean, onPhase?: (event:LifecycleEvent)=>void) {
  ...
  if(grace!==undefined){assertSuccessfulExecution(grace);onPhase?.({phase:'SUCCESS',hash});return {hash,receipt,state,verificationPending:false};}
  onPhase?.({phase:'VERIFICATION_DELAYED',hash});
  return {hash,receipt,state,verificationPending:true};
}
