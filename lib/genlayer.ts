...
export type ActivityRecord={kind:string;raw:unknown};
export type ActivityEvent={event:string;id:string;timestamp:number;status?:string;href:string};
export function aggregateActivityEvents(records:ActivityRecord[]):ActivityEvent[]{const out:ActivityEvent[]=[];for(const item of records){let value:any;try{value=typeof item.raw==='string'?JSON.parse(item.raw):item.raw}catch{continue}if(!value||typeof value!=='object')continue;const id=String(value.plan_id??value.escrow_id??value.receipt_id??value.id??'');if(!id)continue;const timestamp=Number(value.created_at??value.issued_at??value.submitted_at??value.timestamp??0);const status=value.status??value.current_status;const event=item.kind==='plan'?'PLAN_CREATED':item.kind==='escrow'?`ESCROW_${String(status??'CREATED').toUpperCase()}`:item.kind==='receipt'?'AUTHORIZATION_RECEIPT_CREATED':'CANONICAL_EVENT';const href=item.kind==='plan'?`/plans/${id}`:item.kind==='escrow'?`/escrow/${id}`:`/receipts/${id}`;out.push({event,id,timestamp,status,href})}return out.sort((a,b)=>b.timestamp-a.timestamp)}
