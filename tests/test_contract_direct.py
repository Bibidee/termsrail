...
def test_receipts_supersede_and_escrow_expiry_is_terminal(direct_deploy, direct_vm):
    ...
    expired=c.create_escrow(pid,"0xrecipient",1,int(time.time())+1); time.sleep(2); assert c.expire_escrow(expired)=="EXPIRED"; assert c.is_escrow_executable(expired) is False
