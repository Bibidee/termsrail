...
def test_other_dispute_requires_explicit_bounded_choice(direct_deploy, direct_vm):
    response={d:"ALLOWED" for d in DIMENSIONS}; direct_vm.mock_web(r"other-dispute",{"status":200,"body":"allowed terms"}); direct_vm.mock_llm(r"classifying hostile policy evidence",json.dumps(response)); c=direct_deploy(str(CONTRACT),sdk_version="v0.2.12"); sid=c.register_service("other-dispute","Other Dispute","other-dispute.example.com","https://other-dispute.example.com/p","TERMS_OF_SERVICE",86400); c.build_policy_snapshot(sid); aid=c.register_action(sid,"other-dispute-action","OTHER","work",fields()); c.authorize_action(aid); pid=c.create_plan("Other Dispute","bounded choice",json.dumps([{"service_id":sid,"action_id":aid}])); c.authorize_plan(pid); eid=c.create_escrow(pid,"0x0000000000000000000000000000000000000002",1,2**31); fund_escrow(c,direct_vm,eid); did=c.open_dispute(eid,"payer requests a refund because evidence is insufficient"); direct_vm.mock_llm(r"Classify this bounded dispute outcome",json.dumps({"verdict":"OTHER"})); assert c.adjudicate_dispute(did)=="OTHER"; with direct_vm.expect_revert("invalid dispute choice"): c.resolve_dispute_choice(did,"OTHER"); assert c.resolve_dispute_choice(did,"REFUND")=="RESOLVED_REFUND"; assert '"settlement_status": "REFUND_TO_PAYER"' in c.get_escrow(eid)
